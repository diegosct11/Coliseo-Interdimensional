...
def validar_combatiente(combatiente):            
    if combatiente == "A":
...
if "ocupados" not in st.session_state:                
    st.session_state.ocupados = {
        "combatientes" : {
            "con_poderes": [],
            "sin_poderes": []
...
if "categoría_anterior" not in st.session_state:               
    st.session_state.categoría_anterior = None
...
if Fecha != st.session_state.fecha_anterior:
    ...
    armas_ocupadas = []
    if len(st.session_state.combates_planificados.keys()) != 0:
        for combate in st.session_state.combates_planificados.values():         
    ...
    if len(salas_ocupadas) == 0:                                     
        ...
        st.session_state.libres["combatientes"]["con_poderes"] = [combatiente for combatiente, info in st.session_state.recursos["combatientes"].items() if info == "Con Poderes"]
        st.session_state.libres["combatientes"]["sin_poderes"] = [combatiente for combatiente, info in st.session_state.recursos["combatientes"].items() if info == "Sin Poderes"]
        ...
        st.error("Todas las salas están llenas ese día, busca otro para la :red[Destrucción].")
        st.session_state.pelea["Fecha"] = None
        st.session_state.libres = {
            "combatientes" : {
                "con_poderes": [],
        ...
        st.session_state.libres["combatientes"]["con_poderes"] = [combatiente for combatiente, info in st.session_state.recursos["combatientes"].items() if info == "Con Poderes" and combatiente not in combatientes_ocupados["con_poderes"]]
        st.session_state.libres["combatientes"]["sin_poderes"] = [combatiente for combatiente, info in st.session_state.recursos["combatientes"].items() if info == "Sin Poderes" and combatiente not in combatientes_ocupados["sin_poderes"]]
...
elif st.session_state.pelea["Fecha"] == None:                         
    dia_libre = datetime.datetime.today().date()
    if len(st.session_state.combates_planificados.keys()) != 0:
        c = 0
        comp = False
        while True:  
            for pelea in st.session_state.combates_planificados.values():    
                if pelea["Fecha"] == str(dia_libre):
                    if c < 3:
                        c +=1          
                if c == 3:
                    dia_libre = dia_libre + datetime.timedelta(days=1)
                    comp = False
                    c = 0
                    break  
                comp = True    
            if comp:
                break 
    st.info(f"Hueco disponible para una Masacre: {str(dia_libre)}")
...
if st.session_state.pelea["Sala"] == "Sala Ciudad":
    st.image("images/salas/sala ciudad.jpg")
elif st.session_state.pelea["Sala"] == "Sala Bosque":
    st.image("images/salas/sala bosque.jpg")
elif st.session_state.pelea["Sala"] == "Sala Desierto":
    st.image("images/salas/sala desierto.jpg")
else:
    st.session_state.pelea["Sala"] = None
...
if st.session_state.pelea["Categoría"] != st.session_state.categoría_anterior:        
    st.session_state.pelea["Combatiente_A"] = None
    st.session_state.pelea["Combatiente_B"] = None
    st.session_state.pelea["Arma_A"] = None
    st.session_state.pelea["Arma_B"] = None
...
if bt_confirmar:                                
    valor, campos = validar_contienda()
    if not valor:
        for error in campos:
